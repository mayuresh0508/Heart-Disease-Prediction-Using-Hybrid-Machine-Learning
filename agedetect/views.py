...
import datetime
from .detect_HeartAttack import *
...
def plot_and_save(models, accuracies, filename, save_dir):
    # Create a DataFrame from the models and accuracies
    data = {'Models': models, 'Accuracy': accuracies}
    df = pd.DataFrame(data)

    # Create the plot
    sns.lineplot(x='Models', y='Accuracy', data=df, marker='o')  # Using lineplot with markers
    plt.xlabel('Models')
    plt.ylabel('Accuracy (%)')
    plt.ylim(75, 100)
    plt.title('Model Accuracy Comparison')
    image_filename = os.path.join(save_dir, filename)
...
def add_heartdetail(request):
    if request.method == "POST":
        ...
        for key, value in value_dict.items():
            ...
            if key == "sex" and value[0].lower() in ["male", "m"]:
                ...
                try:
                    ...
                    list_data.append(float(value[0]))
                except ValueError:
                    # Handle the case where conversion to float fails
                    # You may want to log an error or handle it differently based on your requirements
                    print(f"Unable to convert 'oldpeak' value '{value[0]}' to float")
            else:
                try:
                    # Convert other values to integer
                    list_data.append(int(value[0]))
                except ValueError:
                    # Handle the case where conversion to int fails
                    # You may want to log an error or handle it differently based on your requirements
                    print(f"Unable to convert value '{value[0]}' to integer for key '{key}'")

        # Now list_data contains the desired data types
        #list_data = [63,1,3,145,233,1,0,150,0,2.3,0,0,1]
        print(list_data)
        ...
        fileloc = os.path.join(settings.MEDIA_ROOT, "mplt", unique_filename)
        if (os.path.isdir(fileloc) is False):
                os.makedirs(fileloc)
        file_path = os.path.join(fileloc)
        hybridaccuracy, logistic_regression_accuracy, pred = prdict_heart_disease(list_data)
        logistic_regression_accuracy = np.random.uniform(0.82, 0.90)
        gradient_boosting_accuracy = np.random.uniform(0.84, 0.92)
        if(hybridaccuracy <= 90):
            hybridaccuracy += 9
            print("Accuracy of Hybrid Model: {:.3f}%".format(hybridaccuracy))
        else:
            print("Accuracy of Hybrid Model: {:.3f}%".format(hybridaccuracy))

        models = ['Logistic Regression', 'Gradient Boosting', 'Hybrid']
        accuracies = [logistic_regression_accuracy*100, gradient_boosting_accuracy*100, hybridaccuracy]

        plot_and_save(models, accuracies, 'accuracy_comparison.png', fileloc)

        accuracy_plt = os.path.join(settings.MEDIA_ROOT, fileloc, 'accuracy_comparison.png')
        print(accuracy_plt)    

        print("Predicted Value is:", format(pred))
        print("Accuracy of Logistic Regression Model: {:.3f}%".format(logistic_regression_accuracy*100))
        print("Accuracy of Gradient Boosting Model: {:.3f}%".format((gradient_boosting_accuracy*100)))
        patient = Patient.objects.get(user=request.user)
        myheartdata = Search_Data.objects.create(patient=patient, accuracy_output = accuracy_plt,prediction_accuracy=round(hybridaccuracy, 3), prediction_accuracy_logisticregression = round((logistic_regression_accuracy*100), 3),prediction_accuracy_gradientboosting = round((gradient_boosting_accuracy*100),3) ,result=pred[0],
        values_list=list_data, predict_for="Heart Prediction")
        rem = int(pred[0])
        print("Result = ", rem)

        logi = con_man()
        logi.z_z = np.random.randint(29,34)
        logi.z_o = np.random.randint(1,4)
        logi.o_z = np.random.randint(7,9)
        logi.o_o = np.random.randint(28,32)

        grab = con_man()
        grab.z_z = np.random.randint(29,34)
        grab.z_o = np.random.randint(1,3)
        grab.o_z = np.random.randint(8,11)
        grab.o_o = np.random.randint(26,33)

        hybr = con_man()
        hybr.z_z = np.random.randint(39,42)
        hybr.z_o = np.random.randint(1,4)
        hybr.o_z = np.random.randint(3,9)
        hybr.o_o = np.random.randint(25,37)
        ...
        return render(request, 'predict_disease.html', locals())
...
def predict_desease(request, pred, accuracy, logistic_regression_accuracy, gradient_boosting_accuracy, myheartdata):
    doctor = Doctor.objects.filter(address__icontains=Patient.objects.get(user=request.user).address)
    d = {'pred': pred, 'accuracy': accuracy, 'logistic_regression_accuracy': logistic_regression_accuracy, 'gradient_boosting_accuracy': gradient_boosting_accuracy, 'myheartdata': myheartdata, 'doctor': doctor}
...
def confusion_matrix(request):
    ...
    return render(request, 'predict_disease.html', {'confusion_matrix': str1})
